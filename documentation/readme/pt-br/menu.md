...
ℹ️ Observação: caso queira que o nome exibido no aplicativo de autenticação (ex.: Config Editor: usuario) seja atualizado, será necessário reescanear o QR Code disponível na tela Autenticação de 2 fatores.
...
⚠️ Esse item só aparece se a variável de ambiente **TOTP_ENABLED** estiver definida como `true`.
...
- A alteração é imediata e aplicada a toda a interface, mesmo se não clicar em `Continuar`.
